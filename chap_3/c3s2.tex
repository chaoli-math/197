...
在于，我们传统地用$Y$上的矢量丛来代替它们的截面层。

为看看这些层是什么样子，首先考虑如果$\mathscr E$是概形$X$上
秩为$1$的平凡矢量丛，那么$\mathscr E$的截面就是$X$上的函数，
因此$\mathscr E$的截面曾应该是$\mathscr O_X$.
取直和，我们可以看到，秩$m$的平凡矢量丛的截面层是凝聚层，自由
$\oo_X$-模$\oo_X^m$. 一般来说，由于矢量丛由定义局部平凡，
它们的截面曾是有限秩的局部自由$\oo_X$-模层，局部自由凝聚层。
反过来，从一个局部自由凝聚层得到一个矢量丛也并不困难。

既然有了矢量丛和局部自由凝聚层的等价，为什么要用局部自由层呢？
这与用概形代替簇的理由是类似的，即使我们主要感兴趣的是簇：
局部自由凝聚层自然地活在更大的凝聚层范畴中，在更大的范畴中工作
给了我们更大的弹性。在较小范畴中的标准构造（比如取一个概形态射的
纤维，或取一个局部自由层同态的余核），在较大范畴中才能最自然地诠释。

就像对应的线丛，秩为$1$的局部自由层起来非常重要的作用且有一个
特别的名字：它们被称为\textit{可逆层}。这个属于来自于数论：
...
上的可区分可逆层的存在性，即对应于恒等映射的那个。在这节中，我们将
给出这个层的描述，其在射影几何中起了基础的作用。
