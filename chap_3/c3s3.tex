...
$R$的自由消解的分次Betti数}，使用它，Hilbert函数和Hilbert
多项式都可以被改写为更方便的形式。（Hilbert多项式比起
Hilbert函数真正的优点是它包含的信息依赖于稍少一些$X$的嵌入
细节，这点下面我们会详细阐述。）

\subsection{自由消解}\label{s:3.3.3}
...
\[
	S(-b)_\nu =S_{\nu-b}
\]
所补偿。下面我们将用分次自由模来消解$R$，或实际上任意的分次
...
令$S=K[x_0,\dots,x_r]$. 在上面的任意极小自由消解中，对某个
$i\leq r+1$，即某个小于变量个数的数，$\varphi_i$是一个单射，
特别地，任意分次$S$-模有一个有限、分次、自由消解。
\end{thm}
...
Matsumura [1986, Section 19] 上关于局部环上的极小自由消解的
讨论，它可以直接翻译到分次的情况。
...
因为$L$和$P$没有公共因子，我们看到，$S/I(X)$的极小自由消解
具有形式
\[
...
现在来计算$S/I(X)$的自由消解。取由四点给出的四次型的两对对边，
这给出两个二次型$q_1$和$q_2$，它们在$X$的理想中没有公共因子。
\inclugra{8.png} % p.136
因为$q_1$和$q_2$是互素的，它们生成的$I$的自由消解具有形式
\[
...
\paragraph*{例子：在
\texorpdfstring{$\mathbb P_K^3$}{PK3}中的一般的双重直线}
\addcontentsline{toc}{subsubsection}{例子：在
\texorpdfstring{$\mathbb P_K^3$}{PK3}中的一般的双重直线}
...
绕着它的既约直线有多快。为证明它，我们将证明$X$的整体截面环
...
\]
所以$f(1/v')=a(v')$，其只有当$f$和$a$都是常数且$f=a$
...
证明了其平坦极限是非既约的：它的支集是两个直线的并，但在它们的
...
既约的；用经典拓扑的语言，映射$\varphi$限制在$U$的原像上是一个
...
实际上，\textit{这个序列是坐标环$S_\Gamma$的一个自由消解}。
...
$\prod d_i$个既约线性子空间的并，因此有着次数$\prod d_i$.
...
是整个$\mathbb P_K^4$，于是$X\cap Y$的Zariski切空间
...
\begin{exe}\label{exe:3.81}
	若在Example \ref{exa:3.79} 中取$X$为既约的会让读者觉得
...
作为我们关于Hilbert方程、Hilbert多项式和自由消解的最后
...
\]
因此，如果我们有概形$X\subset \P_K^n$的一个自由消解
\[
