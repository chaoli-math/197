...
$I(X)$并不由其包含的两个独立二次形式所生成，因为它们有一个
公共因子，于是，他的极小生成元集包含被二次形式，还有一个其他的
生成元，它是不包含于两个二次形式生成的理想中的最低可能次数形式，
...
注意到，存在二次形式（于是更高次形式）包含四点的任意子集而不包含最后一点，这可以同之前一样用直线的并来构造。

现在来计算$S/I(X)$的自由分解。取由四点给出的四次形式的两对对边，
这给出两个二次形式$q_1$和$q_2$，它们在$X$的理想中没有公共因子。
...
Exercise \ref{exe:2.35} 叫你去证明，所有的仿射双重直线是等价的。
