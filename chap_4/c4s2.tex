...
\begin{exe}\label{exe:4.38}
	记号同上题，令$Y\to \mathbb A_K^3$为$\mathbb A_K^3$沿着直线$L$的爆破，$\tilde M\subset Y$为$M$在$Y$中的真变换，$X'\to Y$为$Y$沿着$\tilde M$的爆破。证明，复合$X'\to Y\to \mathbb A_K^3$经由爆破$X=\Bl_N \mathbb A_K^3\to \mathbb A_K^3$分解，诱导的映射$X'\to X$是一个通常的二次点$p\in X$的小消解。
\end{exe}
