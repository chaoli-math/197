...
若我们以具有约化中心的爆破来表$Z$，我们可以看到更多。粗略地说，在经典语言中，$Z$的“”为（见下图）：首先，令$Z_1$为$\mathbb A_K^2$在原点处的爆破，令$E\subset Z_1$为例外除子，即原点的原像。令$P$为$E$在$x$-轴的真变换中的点，及$x$-轴在$Z_1\setminus E$中原像的闭包。令$Z_2$为$Z_1$在点$P$的爆破，令$F\subset Z_2$为这个爆破的例外除子，以及（稍微混用一下记号）$E\subset Z_2$为$E$在$Z_2$中的真变换。于是，在经典语言中，$Z=\operatorname{Bl}_\Gamma (\mathbb A_K^2)$是由$Z_2$ 吹倒 $E$ 得到的，换句话说：\nottran
...
\paragraph*{多次点的爆破}\addcontentsline{toc}{subsubsection}{多次点的爆破} 我们这里将考虑一些平面沿着支于一点的子概形的爆破的例子。
% p.182
