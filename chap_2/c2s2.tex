...
\begin{exe}
	...
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[(a)] $[(0)]$，它的闭包是整个$\mathbb{A}^2_{\rr}$，或者
		\item[(b)] 点$[(f)]\in \mathbb{A}^2_{\rr}$对应于不可约多项式$f\in \rr[x,y]$.
	\end{itemize}
...
\begin{exe}
	...
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[(a)] $(x-\sqrt{2},y-\sqrt{2})$
		\item[(b)] $(x-\sqrt{2},y-\sqrt{3})$
		\item[(c)] $(x-\zeta,y-\zeta^{-1})$，其中$\zeta$是$p$次单位根，而$p$是一个素数
		\item[(d)] $(\sqrt{2}x-\sqrt{3}y)$
		\item[(e)] $(\sqrt{2}x-\sqrt{3}y-1)$
	\end{itemize}
	如果可能则做出图像。
\end{exe}

\begin{exe}
	我们说子概形$X\subset \mathbb{A}_K^n$是\idx{绝对不可约的}或者是\idx{几何不可约的}如果$X$在$\mathbb{A}_{\bar K}^n$中的原像是不可约的。（更一般地，我们说任意$K$-概形$X$是绝对不可约的如果纤维积$X\times_{\spec K}\spec \bar{K}$是不可约的。）分类下面这些$\mathbb{A}_\mathbb{Q}^2=\spec \mathbb{Q}[x,y]$子概形是可约的、不可约但不是绝对不可约的或者绝对不可约的。
	...
	\begin{itemize}\setlength{\itemsep}{0pt}
		...
		\item[(c)] $V(x^2+y^2+1)$
		\item[(d)] $V(x+y,xy-2)$
		\item[(e)] $V(x^2-2y^2,x^3+3y^3)$
	\end{itemize}
\end{exe}

最后，这里有一个联合了局部概形与非代数闭域上的概形两个概念的例子。经典地，平面曲线$X\subset \mathbb{A}_\cc^2$被称为在原点处有一个\idx{结点}，如果在原点处有一个解析邻域，其中$Y$的轨迹包含两个光滑弧在$(0,0)$处横截相交。在概形的语言中，这等价于说，$X$与形式邻域$\spec \cc[\![x,y]\!]\to \spec \cc[x,y]=\mathbb{A}_\cc^2$的纤维积同构于$\spec \cc[\![u,v]\!]/(uv)$.
...
\]
可能有下面两种不同构的形式中的一个：他可能同构于$\spec \rr[\![u,v]\!]/(uv)$或者同构于$\spec \rr[\![u,v]\!]/(u^2+v^2)$. 前一种情况实如果$X$的实点的曲线在$(0,0)$的解析邻域中看此来是两条光滑实弧线横截相交，这样一个点曾被叫做$X$的\idx{叉点}. 后一种情况是原点作为$X$的实点被孤立了，这样点曾经被叫做\idx{孤立点}。
