...
% \begin{wrapfigure}{r}{160pt}\includegraphics[scale=0.5]{chap_2/pics/21.png}\end{wrapfigure}
...
\]
若$Z\subset X$是一个支撑于一点的闭子概形，关联于素理想$(p,f)$，正如在域上的有限子概形那般，我们希望谈论$Z$的度。在域上的概形中，我们定义度为$\oo_Z(Z)$作为$K$\hyp 矢量空间的维度。但是现在$\oo_Z(Z)$可能不再能包含任意的域，比如它可能是$\zz/(p^2)$. 更加令人困惑的时候，它的剩余类域可能不是$\zz/(p)$. 因此，摆脱手头这困境最实惠的方式是，首先注意到基数$\# \oo_Z(Z)$一定具有形式$p^d$，进而可以我们可以取度为$d$，显然，如果$\oo_Z(Z)$是一个$\zz/(p)$\hyp 矢量空间的话，这就是它的维度。（一个更精致的方法是，先定义约态闭点的度为$\zz/(p)$\hyp 矢量空间的维度，然后定义$Z$的度为约态带你的度乘以$Z$在该点的重数。）
