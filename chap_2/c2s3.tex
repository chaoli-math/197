...
回忆在第 \pageref{deg} 页，我们定义了一个有限仿射$K$\hyp 概形$X=\spec R$的\textit{次数}，它是$R$作为有限维$K$\hyp 矢量空间的维度。当$K$是代数闭的时候，这样一个概形$X$的次数，从某种角度来说，度量了它的非既约的程度。然而，就像最后一个习题所展现的，在一般的情况下这并不正确：$\spec \cc$是既约的，但是作为$\mathbb R$上的概形，它的次数为$2$.
