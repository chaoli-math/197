...
\begin{proof}[Proposition \ref{pro:1.1.18}的证明]
...
\]
同样因为已经假设$\{X_{f_a}\}$是有限覆盖，所以对所有的指标，都可以选成一个$N$. 从Lemma \ref{lem:1.1.16}，$f_a\in R$生成单位理想，于是$f_a^N$也生成单位理想%\footnote{译者注：注意到$X_{f_a^N}=X_{f_a}$是$X$的一个覆盖。}
，于是我们可以记
\[
	1=\sum_a e_a f_a^N,
\]
