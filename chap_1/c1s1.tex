...
一般地，如果$\pi:X\to Y$是任意拓扑空间之间的映射，我们定义$\pi$的界面层$\mathscr{J}$通过，对$X$的每个开集$U$，我们定义$\mathscr{J}(U)$是所有使得$\pi\circ \sigma=1$的连续函数$\sigma:U\to \pi^{-1}U$的集合，其中$1$是$U$上的恒同映射（这样的映射被称为集合论意义上的$\pi$的截面：对任意层$\ff$，$\ff(U)$中元素被称为截面正是从这个例子中推广而来的）。
