...
\begin{exe}\label{exe:1.26}
	...
	\]
	但是没有其他包含$X_i\subset X_j$成立，尽管$X_2$和$X_3$的支集相同，且$X_1$的支集包含其中。
\end{exe}
...
\begin{defi}
一个概形$X$和$Y$之间的\textit{态射}，或\textit{映射}是一个有序对$(\psi,\psi^\#)$，其中$\psi:X\to Y$是一个支集之间的连续映射而
\[
	\psi^\#:\oo_Y\to \psi_*\oo_X
\]
是一个$Y$上的层之间的映射，满足对每一点$p\in X$以及$Y$中$q=\psi(p)$的任意邻域$U$，一个截面$f\in\oo_Y(U)$在$q$为零当且仅当$\psi_*\oo_X(U)=\oo_X(\psi^{-1}U)$上的截面$\psi^\# f$在点$p$为零。
\end{defi}
...
如果$\psi:Y\to X$是一个仿射概形间的态射，$X=\spec R$和$Y=\spec T$，$X'$是由$R$的理想$I$定义的$X$的闭子概形，于是，我们定义$\psi$在$X'$上的原像（有时，为了强调这个构造，我们会说“概形意义上的原像”）$\psi^{-1}X'$为$Y$中由$T$的理想$\varphi(I)T$定义的闭子概形。如果$X'$是$X$的一个闭点$p$，我们称$\psi^{-1}p$是$X'$上的\textit{纤维}。（我们马上就会看到如何在任意点上定义纤维。）原像的支集就是集合意义上的原像，然而

\begin{exe}\label{exe:1.43}
\begin{compactenum}[(a)]
\item 令$\varphi:X\to Y$是如下展示的仿射概形之间的映射
\begin{center}\includegraphics[scale=\scale,bb=0 0 257 227]{\PICDIR/1.png}\end{center}
即，$X=\spec K[x,u]/(xu)$是两条在点$p=(x,u)$处相交的直线的交，其中$Y=\spec K[t]$是一个直线
\item 令$\varphi:X\to Y$是如下展示的仿射概形之间的映射
\begin{center}\includegraphics[scale=\scale,bb=0 0 320 352]{\PICDIR/2.png}\end{center}
\end{compactenum}
\end{exe}
...
\begin{center}\includegraphics[scale=\scale,bb=0 0 562 68]{\PICDIR/3.png}\end{center}
